...
void printUsage() {
  ...
  printf("Usage: ttmatvec -a [file-name-matrix-A] -x [file-name-vector-x] -y "
         "[file-name-vector-y]\n");
...
int main(int argc, char **argv) {
  ...
  double makespan = time_span.count();
  printf("makespan %e seconds\n", makespan);
  double MOPS = 0.0;
  for (int d = 0; d < A.m[d]; d++) {
    MOPS += A.m[d] * A.n[d] * A.r[d] * A.r[d + 1] * x.r[d] * x.r[d + 1];
  }
  std::cout << "MOPS " << MOPS << std::endl;

  double GFLOPS = MOPS / 1e9 / makespan;
