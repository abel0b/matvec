...
void loadTTMat(char *fileName, TTMat *mat) {
  ...
  mat->m = (int *)malloc(
      mat->d * sizeof(mat->m[0])); // Number of matrix rows in each dimension
  fread(mat->m, sizeof(mat->m[0]), mat->d, file);
  mat->n = (int *)malloc(
      mat->d * sizeof(mat->n[0])); // Number of matrix columns in each dimension
  fread(mat->n, sizeof(mat->n[0]), mat->d, file);
  mat->r = (int *)malloc((mat->d + 1) * sizeof(mat->r[0]));
  mat->r[0] = mat->r[mat->d] =
      1; // The first and the last ranks are always one.
  fread(mat->r + 1, sizeof(mat->r[0]), mat->d - 1,
        file); // Read the remaining D - 1 ranks
  mat->dimMatBegin =
      (size_t *)malloc((mat->d + 1) * sizeof(mat->dimMatBegin[0]));
  mat->dimMatBegin[0] = 0;
  for (int d = 0; d < mat->d; d++) { // Setup pointers for the beginning of data
                                     // for the matrix of each dimension
    // Each dimension has a matrix of size m[d] x n[d], each element of this
    // matrix is of size r[d] x r[d + 1].
    mat->dimMatBegin[d + 1] =
        mat->dimMatBegin[d] + mat->m[d] * mat->n[d] * mat->r[d] * mat->r[d + 1];
  }
  mat->data = (double *)malloc(mat->dimMatBegin[mat->d] *
                               sizeof(mat->data[0])); // Allocate all matrices
  fread(mat->data, sizeof(mat->data[0]), mat->dimMatBegin[mat->d],
        file); // Read the input
  fclose(file);
}
  ...
  for (int d = 0; d < mat->d; d++) {
    ...
    for (int i = 0; i < mat->m[d]; i++) {
      for (int j = 0; j < mat->n[d]; j++) {
        ...
        double *blockBegin = mat->data + mat->dimMatBegin[d] +
                             (i + j * mat->m[d]) * (mat->r[d] * mat->r[d + 1]);
...
#endif
    double *
getTTMatBlock(TTMat *mat, int dim, int rowIdx, int colIdx) {
  return mat->data + mat->dimMatBegin[dim] +
         ((size_t)(rowIdx + colIdx * mat->m[dim]) *
          (size_t)(mat->r[dim] * mat->r[dim + 1]));
}
...
#endif
    void
multiplyAddKronecker(double *a, int anrows, int ancols, double *b, int bnrows,
                     int bncols, double *c) {
    ...
    for (int ia = 0; ia < anrows; ia++) {
      ...
      for (int jb = 0; jb < bncols; jb++) {
        for (int ib = 0; ib < bnrows; ib++) {
          ...
          size_t cElemIdx =
              (ia + ja * anrows) * bnrowsXbncols + ib + jb * anrowsXbnrows;
  ...
  for (int d = 0; d < y->d; d++) {
    for (int m = 0; m < A->m[d]; m++) {
      ...
      for (int n = 0; n < A->n[d]; n++) {
        ...
        multiplyAddKronecker(AmnBlockBegin, A->r[d], A->r[d + 1], xnBlockBegin,
                             x->r[d], x->r[d + 1], ymBlockBegin);
      }
    }
  }
}
