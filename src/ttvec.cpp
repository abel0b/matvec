...
void loadTTVec(char *fileName, TTVec *vec) {
  ...
  vec->dimVecBegin =
      (size_t *)malloc((vec->d + 1) * sizeof(vec->dimVecBegin[0]));
  vec->dimVecBegin[0] = 0;
  for (int d = 0; d < vec->d; d++) {
    vec->dimVecBegin[d + 1] =
        vec->dimVecBegin[d] + vec->m[d] * vec->r[d] * vec->r[d + 1];
  }
...
void printTTVec(TTVec *vec, FILE *outFile) {
  ...
  for (int d = 0; d < vec->d; d++) {
    ...
    for (int i = 0; i < vec->m[d]; i++) {
      ...
      double *blockBegin =
          vec->data + vec->dimVecBegin[d] + i * (vec->r[d] * vec->r[d + 1]);
...
double *getTTVecBlock(TTVec *vec, int dim, int rowIdx) {
  return vec->data + vec->dimVecBegin[dim] +
         ((size_t)rowIdx * vec->r[dim] * vec->r[dim + 1]);
}
...
int compareTTVec(TTVec *x, TTVec *y) {
  ...
  for (int d = 0; d < x->d; d++) {
    ...
    if (x->dimVecBegin[d] != y->dimVecBegin[d]) {
      return 0;
    }
  }
  if (x->dimVecBegin[x->d] != y->dimVecBegin[y->d]) {
    return 0;
  }
  double sqError = 0.0;
  double normX = 0.0;
  ...
  //  printf("error %e\n", sqError);
  if (maxNorm < 1e-6) {
    return (sqError < 1e-12);
  } else {
    return (sqError / maxNorm < 1e-6);
  }
}
